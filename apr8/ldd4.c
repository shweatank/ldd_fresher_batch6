...
static ssize_t basic_write(struct file *file,const char __user *user_buffer,size_t count, loff_t *offset)
{
	...
	int i=0;
	int num1=0,num2=0;
	while(kernel_buffer[i]>='0' && kernel_buffer[i]<='9')
	{
	...
	int j=0;
	char op[10];
	while(kernel_buffer[i]!='\0' && kernel_buffer[i]!='\n')
	{
		op[j++]=kernel_buffer[i];
		i++;
	}
	op[j]='\0';
	int result;
	...
	while(start<end)
	{
		char temp=kernel_buffer[start];
		kernel_buffer[start]=kernel_buffer[end];
		kernel_buffer[end]=temp;
		start++;
		end--;
	}
	kernel_buffer[i]='\0';
