...
static irqreturn_t irq_demo_isr(int irq, void *dev_id)
{
	...
	pr_info("Keyboard Interrupt Occured and the value is %d",demo_value);
        return IRQ_HANDLED;
}
...
static int __init sysfs_demo_init(void)
{
	...
	int ret= request_irq(IRQ_NUM,irq_demo_isr,IRQF_SHARED,"sysfs_demo",(void *)irq_demo_isr);
        if(ret)
        {
                pr_err("Failed to request ISR %d\n",IRQ_NUM);
                return ret;
        }
