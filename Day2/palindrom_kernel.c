...
static ssize_t basic_write(struct file *file,const char __user *user_buffer ,size_t count,loff_t *offset)
{
     ...
     int i=0,j=strlen(kernel_buffer)-1,flag=0;
     while(i<j)
     { 
     ...
     if(flag==0)
	  strscpy(driver_msg,"palindrome",sizeof(driver_msg));
     else
	  strscpy(driver_msg,"not a palindrome",sizeof(driver_msg));
...
MODULE_DESCRIPTION("Checking if a string is palindrome");
