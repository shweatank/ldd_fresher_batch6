...
#define DEVICE_NAME "max_using_tasklet"
...
static void keyboard_tasklet_fn(struct tasklet_struct *t)
{
	...
	{
		kernel_value=user_value.num1;
	}
	else
	{
		kernel_value=user_value.num2;
	}
...
{
	pr_info("Threaded IRQ: result is %d\n",kernel_value);
	return IRQ_HANDLED;
}
...
static int __init tasklet_irq_init(void)
{
	...
	request_threaded_irq(KEYBOARD_IRQ,keyboard_irq_handler,irq_thread,IRQF_SHARED,"irq_threaded",(void *)irq_thread);
