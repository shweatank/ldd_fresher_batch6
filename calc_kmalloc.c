...
static ssize_t basic_write(struct file *file,const char __user *user_buffer, size_t count, loff_t *offset)
{
	...
	int num1,num2,res;
        char *temp_buffer=(char *)kmalloc(count*sizeof(char),GFP_KERNEL);
	if(!temp_buffer)
	{
		printk(KERN_ALERT "Memory allocation failed\n");
		return -ENOMEM;
	}
	kernel_buffer=(char *)kmalloc(count*sizeof(char),GFP_KERNEL);
        if(!kernel_buffer)
        {
                printk(KERN_ALERT "Memory allocation failed\n");
                return -ENOMEM;
	...
	char opp[10];
	sscanf(temp_buffer,"%d %d %s",&num1,&num2,opp);
	if(strcmp(opp,"add")==0)
	{
		res=num1+num2;
	}
	else if(strcmp(opp,"sub")==0)
	{
		res=num1-num2;
	}
	else if(strcmp(opp,"mul")==0)
	{
		res=num1*num2;
	}
	else if(strcmp(opp,"div")==0)
	{
		res=num1/num2;
	}
	else
