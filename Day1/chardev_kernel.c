...
/*
 * Called when user wants string to reverse when received from driver space from  /dev/basic_char
 */
...
static ssize_t basic_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *offset)
{
	...
	//Reverse the string 
	/* int l=0;
         int r=bytes_to_copy-1;
        while(l<r)
        {
        char temp=kernel_buffer[l];
        kernel_buffer[l]=kernel_buffer[r];
        kernel_buffer[r]=temp;
        l++;
        r--;
        }*/
	//reverse_string(kernel_buffer,bytes_to_copy);
        int result=kernel_buffer[0]+kernel_buffer[1];
