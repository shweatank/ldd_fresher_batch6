...
static init__init kmalloc_exampl_init(void){
printk(KERN_INFO "kmallos example:init\n");
...

{
	...
	//op=kernel_buffer[4];
	op=kernel_buffer[len];
	int i;
	for(i=0;kernel_buffer[i]!=',';i++){
	val_1=(val_1*10)+kernel_buffer[i];
	}
	for(i=i+1;kernel_buffer[i]!=',';i++){
	val_2=(val_2*10)+kernel_buffer[i];
	}

	switch(op){
		case '+':
			kernel_buffer[0]=val_1+val_2;
			break;
		case '*':
			kernel_buffer[0]=val_1*val_2;
			break;
