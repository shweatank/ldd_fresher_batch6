/*
 * my_uart_driver.c - A kernel-level secure UART driver using workqueue
 *
 * This driver demonstrates a simple character device that simulates
 * secure communication over UART by applying a basic XOR transformation
 * to the data written by user space. The processing is deferred to a
 * workqueue to avoid blocking the write() path, and read() will block
 * until processed data is available.
 *
 * IOCTL commands allow switching between different modes:
 * - ENCRYPT: applies XOR to "encrypt" data
 * - DECRYPT: applies XOR again to "decrypt" data (same operation for XOR)
 * - PASS: no transformation, just pass through
 *
 * User space can use select/poll/epoll to wait for data availability.
 *
 * Note: This is a demonstration and not intended for real security use.
 */
