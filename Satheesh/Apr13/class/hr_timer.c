...
#define TIMER_INTERVAL_NS 1000000
...
static enum hrtimer_restart hrtimer_callback(struct hrtimer *timer)
{
	...
	// forword the timer to  next period 
...
{
	printk(KERN_INFO"Hign resolution timer driver loaded\n");
	interval=ktime_set(0,TIMER_INTERVAL_NS);//0 sec+ns
	// initialize hrtimer 
	//hrtimer_setup(&my_hrtimer,CLOCK_MONOTONIC,HRTIMER_MODE_REL);
...
MODULE_DESCRIPTION("Linux kernel high resolution timer");
