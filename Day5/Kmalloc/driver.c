                           ...
                           size_t count, loff_t *offset)
{
    ...
    // Allocate memory dynamically for each write or reuse a global pointer
    // For this example, we allocate based on the incoming 'count'
    kernal_buffer = kmalloc(count + 1, GFP_KERNEL);
    if (!kernal_buffer) {
        return -ENOMEM;
    }

    if (copy_from_user(kernal_buffer, user_buffer, count)) {
        kfree(kernal_buffer);
        return -EFAULT;
    }

    kernal_buffer[count] = '\0';

    if (sscanf(kernal_buffer, "%d %d %3s", &a, &b, op) == 3) {
        if (strcmp(op, "add") == 0) {
            result = a + b;
            printk(KERN_INFO "Result: %d + %d = %d\n", a, b, result);
        }
    snprintf(kernal_buffer, strlen(kernal_buffer), "%d", result);
    // Update buffer_size (used by your read function) to the new string length
    buffer_size = strlen(kernal_buffer);
    }
   // kfree(kernal_buffer); 
    return count;
}
