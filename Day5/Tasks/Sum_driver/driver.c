                           ...
                           size_t count, loff_t *offset)
{
    ...
    // Convert string (e.g., "5") to long and add to total
    if (kstrtol(kernal_buffer, 10, &val) == 0) {
        running_total += val;
    }

    // Format the NEW total back into the buffer for the next 'read' call
    buffer_size = snprintf(kernal_buffer, BUF_SIZE, "%ld\n", running_total);
    
    *offset = 0; // Reset offset for the next read
    return bytes_to_copy;
}
