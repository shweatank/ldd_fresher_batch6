...
### Driver & Platform
1. Explain char device registration flow using `alloc_chrdev_region`, `cdev_add`, `class_create`, `device_create`.
2. How does `of_match_table` trigger automatic probe?
3. Why use platform driver for non-discoverable hardware?

### Interrupts & Tasklets
1. What must never be done inside ISR and why?
2. Difference between top half and bottom half.
3. Why tasklet here instead of doing LED/TFT updates directly in ISR?

### Wait Queues
1. Why is wait queue ideal for auth result synchronization?
2. Explain producer-consumer pattern in this project.
3. How would you handle spurious wakeups?

### SPI & ILI9225
1. How are command/data writes distinguished (DC GPIO)?
2. Why use SPI subsystem instead of direct SPI register access?
3. What is the role of init sequence/gamma/power registers?

### Synchronization
1. Why spinlock for auth state and mutex for SPI operations?
2. Atomic vs spinlock - where each is used.
3. How race can occur between ioctl and interrupt contexts.

### Device Tree
1. Why DTS preferred over hardcoded pin/address values?
2. Explain `interrupt-parent` and `interrupts`.
3. How Raspberry Pi loads DTB/DT overlays at boot.

### IOREMAP & MMIO
1. Physical vs kernel virtual address in drivers.
2. Flow: DTS -> resource -> ioremap -> readl/writel.
3. Production tradeoff between framework APIs and raw MMIO.

### Security
1. Why salted hashing instead of plain SHA-256(password)?
2. How lockout policy reduces brute-force risk.
3. Why recovery data should also be hashed.
