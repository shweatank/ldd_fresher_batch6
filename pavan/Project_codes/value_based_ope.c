		...
		size_t count, loff_t *offset)
{
	...
	// Read GPIO value
	gpio_val = gpio_get_value(GPIO_LED);

	// Convert to ASCII
	value = gpio_val ? '1' : '0';

	// EOF handling (only allow one read)
	if (*offset > 0)
		return 0;

	// Copy 1 byte to user
	if (copy_to_user(user_buffer, &value, 1))
		return -EFAULT;

	*offset = 1;

	pr_info("LED: Read GPIO value = %c\n", value);
