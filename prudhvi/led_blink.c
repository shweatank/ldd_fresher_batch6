...
static ssize_t led_read(struct file *file,char __user *user_buffer,size_t count,loff_t *offset)
{
	int value;
	char kbuf[16];
	int ret;
	...
	// copy to user space
	if (copy_to_user(user_buffer, kbuf, ret))
		return -EFAULT;
...
static void __exit led_exit(void) {
    ...
    gpio_direction_input(GPIO_LED);  // Set as input, initial
    gpio_free(GPIO_LED);
